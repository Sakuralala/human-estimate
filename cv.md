...
2018.01.23
1、fcn的训练：和训练一般的分类网络一致,一般使用softmax交叉熵损失函数(若是互斥分类，即一张图里就存在一个类别)：
    Si=e^i/∑j(e^j),即假若存在一个数组表示一张图片属于各类的得分,则该图片属于各类的概率等于e^得分/e^得分的和。
2、fcn的标签制作,看了下几个源码的实现发现logits最后都变为[b,h,w,c]的维度，每个点为概率值，但计算loss时label并没有转换为概率值。。？？下面大概介绍下用于训练的label的生成过程：  
    a.首先，对一张原始图片进行标注，不同的类别标注不同的颜色，然后保存；
    b.然后，在读入一个batch的标注图片xx后，调用rgb2ind(xx,map)得到一个索引图像x，包括数据矩阵xx和调色板(colormap)map，最后调研imwrite(x,map,name)将其转换为需要的标签图像；这时候各个像素点的值范围就是$pixel\in [0,classes)$了。   
    所谓索引图像，即将索引映射到像素值，每一个不同的像素值用一个索引来保存。
...
4、常用的四种交叉熵(cross-entropy)损失函数的定义：  
    首先，最大似然估计(MLE):已知一组数据的观测值x,未知的分布模型model,则有x出现的概率$P(x|model)$,最大似然估计就是寻找模型的一组参数值θ使得出现观测值$x$的概率$P(x|model)$最大化。即求：  
    $argmax_θ(p(x_1|θ)*p(x_2|θ)*p(x_n|θ))$  
    由于连乘可能使得结果接近于0，所以实际操作时一般取对数(对数使得连乘变成连加，而又由于$log$为单调递增，故不会改变极值的位置)  
    其次是相对熵，假设存在两个数据的分布p、q，则其相对熵定义为：  
    $D(p||q)=\sum p(x)log(\frac{p(x)}{q(x)})=\sum p(x)*log(p(x))-\sum p(x)*log(q(x))$;其中$H(p)=\sum p(x)*log(p(x))$称为分布p的熵;  
    而$CE(p,q)=-\sum p(x)*log(q(x))$即称为p、q的交叉熵。  
    然后，one-hot编码，主要是为了将标签转换为数字特征，对一些不存在相应大小关系的标签进行编码,表明属于/不属于。而对于一些标签已经进行过预处理的情况来说，如fcn中标签的标记是一张图片，上面各个类别采用不同的颜色标注，这种就不需要再进行one-hot编码了，因为已经转换成了数字特征。  
    a.logistic回归。  
    ...
    其中z、x若未经过one-hot编码，则适用于传统的二分类问题(单元)，若经过one-hot编码，则适用于多元/单元多分类问题。
    b.softmax。 
    softmax = tf.exp(logits) / tf.reduce_sum(tf.exp(logits), axis)
    适用条件：多分类问题，每个实例属于的类必须唯一,因为softmax函数将各个输入映射到一个概率，且总和为1。
    L=-∑z*log(softmax(x)),其中z、x均已经过one-hot编码。
    tf.nn.sparse_softmax_cross_entropy_with_logits:
    作用和tf.nn.softmax_cross_entropy_with_logits一样，只不过这里输入的label是比logits少一维的，即少了类别维，函数在内部会进行onehot编码。  
    另外，查看tensorflow官方api可以看到，sparse_softmax_cross_entropy_with_logits和softmax_cross_entropy_with_logits均要求类别的互斥唯一性，那么对于fcn来说，这个要求不是就被破坏了？然后在具体查看了下相关源码之后终于明白了，现叙述如下：  
    对于传统的分类问题而言，针对的是图片级别的，即每一个batch的图片,最终经过网络后在softmax前的维度应该为[b,c],对应一个label,维度为[b],然后经过softmax转换为概率值再经过交叉熵计算并取均值得到最终的loss值；而对应fcn而言，本质上它也是一个分类的问题，但针对的是像素级别的分类，对于每个batch的图片，经过网络后输出的是[b,h,w,c]维度的东西，而label的维度为[b,h,w],查看了源码后发现其实只要logits的维度大于2维，那么最终都会被reshape到2维，以上述为例，即[b\*h\*w,c],而label就被reshape到一维，即[b\*w\*c]，然后才进行交叉熵的计算，可以看到，在reshape之后，横轴的数量即为像素点的个数，而纵轴即为类别数，这和传统图片分类问题的区别就是横轴的大小，即传统分类的横轴大小为图片个数，而fcn的为像素点个数，一言蔽之，即fcn是针对每个像素点进行交叉熵的计算的，是像素级别的分类，这也就不存在什么一个像素点存在多个类别的问题了，因为不可能，而传统的分类问题由于是针对图片级别的，所以一个图片可能包含多个类别，这时候如果需要使用softmax相关的话就必须要求互斥唯一性。  
