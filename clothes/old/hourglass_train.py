        ...
        while True:
            try:
                ...
                _, loss_v,output_v3, summ, step = sess.run(
                    [train_op, loss, output[3],merged_op, global_step],
                    feed_dict={x:image_batch},options=run_options,run_metadata=run_metadata)
                ...
                if (step % train_para['show_loss_freq']) == 0:
                    ...
                    for i in range(output_v3.shape[-1]-1):
                            scipy.misc.imsave('.//outimg//pre_4%d.png'%i,output_v3[0,:,:,i])
                            scipy.misc.imsave('.//outimg//gt%d.png'%i,gt_heatmap_batch[0,:,:,i])
