...
class GenerateHeatmap():
    ...
    def __call__(self, keypoints):
        hms = np.zeros(
            shape=(self.num_parts, self.output_res, self.output_res),
            dtype=np.float32)
        sigma = self.sigma
        for p in keypoints:
        ...
        for i in range(len(keypoints)):
            ...
            for idx, pt in enumerate(keypoints[i]):
                ...
                if pt[2] > 0 and x >= 0 and y >= 0 and x < output_res and y < output_res:
                    visible_nodes[i][tot] = (
                        idx * output_res * output_res + y * output_res + x, 1)
                    tot += 1
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, ds, index):
        ...
        self.generateHeatmap = GenerateHeatmap(
            config['train']['output_res'], config['inference']['num_parts'])
        self.keypointsRef = KeypointsRef(config['train']['max_num_people'],
    ...
    def loadImage(self, idx):
        ...
        mat_mask = get_transform(
            center, scale, (self.output_res, self.output_res), aug_rot)[:2]
        mask = cv2.warpAffine((mask * 255).astype(np.uint8), mat_mask,
                              (self.output_res, self.output_res)) / 255
        ...
        return self.preprocess(inp).astype(np.float32), mask.astype(
            np.float32), keypoints.astype(np.int32), heatmaps.astype(
                np.float32)
