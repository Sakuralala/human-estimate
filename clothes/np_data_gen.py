    ...
    def next(self):
        ...
        for i in range(self.batch_size):
            ...
            coor_3d = np.concatenate(
                (coor_xy, np.expand_dims(np.ones(coor_orig.shape[0]), 1)), 1)
            ...
            gt_heatmaps = make_gt_heatmaps(
                coor_trans[:, 0:2], self.resized_size, 1.0, coor_trans[:, 2])
            #缩小到和网络输出相同尺寸 先x后y
